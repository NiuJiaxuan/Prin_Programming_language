...
void executeCall(struct nodeCall* c) {
	pushExecutePop(c->name, c->params, &gcCount);
}
...
void executeAssign(struct nodeAssign* a) {
	if (a->type == 0) {
		...
		// print total reference counting 
		gcCount++;
		printf("gc:%d\n", gcCount);
...
void executeLoop(struct nodeLoop* loop) {
	while (executeCond(loop->cond)) {
		...
		//deal with the while conditions when record vaibales defined in while out of scope
		int temp =getReferenceCount();
		for (int i=0;i<temp;i++){
