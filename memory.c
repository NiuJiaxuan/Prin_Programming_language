...
struct frame {
	...
	//reference counting in current frame
	int referenceCount;
};
...
void pushExecutePop(char* name, char** args,int* gcCount) {
	...
	// delete the record varibale count which defined this function frame
	for(int i = 0; i<callStack[fp]->referenceCount;i++){
		*(gcCount) = *(gcCount)-1;
		printf("gc:%d\n",*(gcCount));
	}
...
void addReferenceCount( ){
	callStack[fp]->referenceCount++;
}

// reference count decrease one
...
int getReferenceCount( ){
	return callStack[fp]->referenceCount;
}